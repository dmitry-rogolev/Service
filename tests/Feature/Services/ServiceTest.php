...
class ServiceTest extends TestCase
{
     ...
     */
    public function test_get_unique_keys(): void
    {
        $this->assertIsArray(Service::getUniqueKeys());
        $this->assertNotEmpty(Service::getUniqueKeys());
    }
    ...
    public function test_where_key_with_one_param(): void
    {
        ...
        $models = Service::whereKey($user->getKey());
        ...
        $model = Service::whereKey($user->getKey())->first();
        ...
        $model = Service::whereKey($user)->first();
        $this->assertTrue($user->is($model));
        ...
        $model = Service::whereKey(364939)->first();
        ...
        $model = Service::whereKey($this->generate(User::class, false))->first();
        $this->assertNull($model);
    ...
    public function test_where_key_with_many_params(): void
    {
        ...
        $condition = Service::whereKey('key', 4564646, null)->isEmpty();
    ...
    public function test_where_unique_key_with_one_param(): void
    {
        ...
        // ! ||--------------------------------------------------------------------------------||
        // ! ||                    Передаем отсутствующую в таблице модель,                    ||
        // ! ||             но имеющую существующее значение уникального аттрибута.            ||
        // ! ||--------------------------------------------------------------------------------||

        $id = Service::getModel()::factory()->make(['email' => $user->email]);
        $model = Service::whereUniqueKey($id)->first();
        $this->assertTrue($user->is($model));
    ...
    public function test_where_unique_key_with_many_params(): void
    {
        ...
        // ! ||--------------------------------------------------------------------------------||
        // ! ||               Передаем коллекцию отсутствующих в таблице моделей,              ||
        // ! ||           но содержащих существующие в таблице уникальные аттрибуты.           ||
        // ! ||--------------------------------------------------------------------------------||

        $ids = [
            $this->generate(User::class, ['email' => $users->get(0)->email], false),
            $this->generate(User::class, ['email' => $users->get(1)->email], false),
            $this->generate(User::class, ['email' => $users->get(2)->email], false),
        ];
        $actual = Service::whereUniqueKey($ids)->pluck('id')->all();
        $this->assertEquals($expected, $actual);
        ...
        $condition = Service::whereUniqueKey('key', 4564646, null)->isEmpty();
        $this->assertTrue($condition);
